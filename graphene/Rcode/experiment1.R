...
path <- "/home/tank/graphene/results/figures/"

dat <- read_delim("/home/tank/graphene/experiment1/results/experiment1-2336830.csv",
                  delim = '\t',
...
#results.3 <- dat %>% group_by(TFPs, blkSize, bound) %>% summarize(size=mean(second_IBLT), type='second_IBLT') # 之前代码是(TFPs, fractionHas),然而列名并没有fractionHas
#results.4 <- dat %>% group_by(TFPs, blkSize, bound) %>% summarize(size=mean(second_BF), type='second_BF')  # 之前代码是(TFPs, fractionHas),然而列名并没有fractionHas
#results.5 <- dat %>% group_by(TFPs, blkSize, bound) %>% summarize(size=mean(extra), type="extra") # experiment1并未生成extra变量
results <- bind_rows(results.1, results.2)#results.3, results.4, 之前写的是仅有results.5
