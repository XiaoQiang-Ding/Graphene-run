...
dat <- read_delim("/home/tank/graphene/ping-pong/results/ping-pong-8231781.csv",
                  delim = '\t',
...
results <- dat %>% group_by(TFPs, blkSize) %>% summarize(avg=mean(fprS), var=var(fprS), cnt=n()) #ci=1.96*sqrt(avg*(1-avg)/n()), )
